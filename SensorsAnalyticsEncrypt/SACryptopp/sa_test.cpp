...
//            scoped_main added to SA_CryptoPP::SA_Test namespace by JW in July 2017
...
void SA_FIPS140_SampleApplication();
void RegisterFactories(SA_CryptoPP::SA_Test::TestClass suites);
int (*SA_AdhocTest)(int argc, char *argv[]) = NULLPTR;
...
int scoped_main(int argc, char *argv[])
{
	...
	try
	{
		...
		else if (command == "ss")
		{
			...
			SA_FIPS140_SampleApplication();
		else if (command == "fips-rand")
			FIPS140_GenerateRandomFiles();
		else if (command == "a")
		{
			if (SA_AdhocTest)
				return (*SA_AdhocTest)(argc, argv);
			else
			{
				std::cerr << "SA_AdhocTest not defined.\n";
				return 1;
			}
		}
...
# if (PATH_MAX > 0)  // Posix
	...
	if (sa_getauxval(AT_EXECFN))
		pathHint = sa_getauxval(AT_EXECFN);
...
// a 'using namespace SA_CryptoPP', it causes compile failures.
// Also see http://github.com/weidai11/cryptopp/issues/442
// and http://github.com/weidai11/cryptopp/issues/447.
int CRYPTOPP_API sa_main(int argc, char *argv[])
{
	return SA_CryptoPP::SA_Test::scoped_main(argc, argv);
}
