...
public:
	...
	FileStore(std::istream &in) : m_stream(NULLPTR), m_space(NULLPTR), m_len(0), m_waiting(0)
		{StoreInitialize(MakeParameters(SA_Name::InputStreamPointer(), &in));}
	...
	FileStore(const char *filename) : m_stream(NULLPTR), m_space(NULLPTR), m_len(0), m_waiting(0)
		{StoreInitialize(MakeParameters(SA_Name::InputFileName(), filename ? filename : ""));}
	...
	FileStore(const wchar_t *filename)
		{StoreInitialize(MakeParameters(SA_Name::InputFileNameWide(), filename));}
#endif
...
public:
	...
	FileSource(std::istream &in, bool pumpAll, BufferedTransformation *attachment = NULLPTR)
		: SourceTemplate<FileStore>(attachment) {SourceInitialize(pumpAll, MakeParameters(SA_Name::InputStreamPointer(), &in));}
	...
	FileSource(const char *filename, bool pumpAll, BufferedTransformation *attachment = NULLPTR, bool binary=true)
		: SourceTemplate<FileStore>(attachment) {SourceInitialize(pumpAll, MakeParameters(SA_Name::InputFileName(), filename)(SA_Name::InputBinaryMode(), binary));}
	...
	FileSource(const wchar_t *filename, bool pumpAll, BufferedTransformation *attachment = NULLPTR, bool binary=true)
		: SourceTemplate<FileStore>(attachment) {SourceInitialize(pumpAll, MakeParameters(SA_Name::InputFileNameWide(), filename)(SA_Name::InputBinaryMode(), binary));}
#endif
...
public:
	...
	FileSink(std::ostream &out)
		{IsolatedInitialize(MakeParameters(SA_Name::OutputStreamPointer(), &out));}

	/// \brief Construct a FileSink
	/// \param filename the narrow name of the file to open
	/// \param binary flag indicating if the file is binary
	FileSink(const char *filename, bool binary=true)
		{IsolatedInitialize(MakeParameters(SA_Name::OutputFileName(), filename)(SA_Name::OutputBinaryMode(), binary));}
	...
	FileSink(const wchar_t *filename, bool binary=true)
		{IsolatedInitialize(MakeParameters(SA_Name::OutputFileNameWide(), filename)(SA_Name::OutputBinaryMode(), binary));}
#endif
