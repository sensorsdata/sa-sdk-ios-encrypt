...
void ModePolicyCommonTemplate<POLICY_INTERFACE>::CipherSetKey(const NameValuePairs &params, const byte *key, size_t length)
{
	...
	ResizeBuffers();
	int feedbackSize = params.GetIntValueWithDefault(SA_Name::FeedbackSize(), 0);
...
protected:
	...
	// sa_adv_simd.h increments the counter
...
protected:
	void UncheckedSetKey(const byte *key, unsigned int length, const NameValuePairs &params)
	{
		...
		m_stolenIV = params.GetValueWithDefault(SA_Name::StolenIV(), static_cast<byte *>(NULLPTR));
	}
...
public:
	...
	{
		this->m_cipher = &this->m_object;
		this->SetKey(key, length, MakeParameters(SA_Name::IV(), ConstByteArrayParameter(iv, this->m_cipher->BlockSize())));
	}
	...
	{
		this->m_cipher = &this->m_object;
		this->SetKey(key, length, MakeParameters(SA_Name::IV(), ConstByteArrayParameter(iv, this->m_cipher->BlockSize()))(SA_Name::FeedbackSize(), feedbackSize));
	}
