...
{
	SetKey(key, length, MakeParameters(SA_Name::Rounds(), rounds));
}

void SimpleKeyingInterface::SetKeyWithIV(const byte *key, size_t length, const byte *iv, size_t ivLength)
{
	SetKey(key, length, MakeParameters(SA_Name::IV(), ConstByteArrayParameter(iv, ivLength)));
}
...
const byte * SimpleKeyingInterface::GetIVAndThrowIfInvalid(const NameValuePairs &params, size_t &size)
{
	...
	try {found = params.GetValue(SA_Name::IV(), ivWithLength);}
	...
	else if (params.GetValue(SA_Name::IV(), iv))
	{
		ThrowIfInvalidIV(iv);
		size = static_cast<size_t>(IVSize());
	}
