...
void FileStore::StoreInitialize(const NameValuePairs &parameters)
{
	...
	if (!parameters.GetValue(SA_Name::InputFileNameWide(), fileNameWide))
#endif
		if (!parameters.GetValue(SA_Name::InputFileName(), fileName))
		{
			parameters.GetValue(SA_Name::InputStreamPointer(), m_stream);
			return;
		}

	std::ios::openmode binary = parameters.GetValueWithDefault(SA_Name::InputBinaryMode(), true) ? std::ios::binary : std::ios::openmode(0);
...
#if defined(CRYPTOPP_UNIX_AVAILABLE) || _MSC_VER >= 1400
	const wchar_t *fileNameWide = NULLPTR;
	if (!parameters.GetValue(SA_Name::OutputFileNameWide(), fileNameWide))
#endif
		if (!parameters.GetValue(SA_Name::OutputFileName(), fileName))
		{
			parameters.GetValue(SA_Name::OutputStreamPointer(), m_stream);
			return;
		}

	std::ios::openmode binary = parameters.GetValueWithDefault(SA_Name::OutputBinaryMode(), true) ? std::ios::binary : std::ios::openmode(0);
