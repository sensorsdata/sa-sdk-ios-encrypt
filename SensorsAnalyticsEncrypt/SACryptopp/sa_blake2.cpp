...
using SA_CryptoPP::word64;
using SA_CryptoPP::rotrConstant;
    ...
    : m_digestSize(digestSize), m_keyLength(0), m_treeMode(treeMode)
{
    ...
    UncheckedSetKey(NULLPTR, 0, MakeParameters
        (SA_Name::DigestSize(), (int)digestSize)
        (SA_Name::TreeMode(), treeMode));
}
    ...
    : m_digestSize(digestSize), m_keyLength(static_cast<unsigned int>(keyLength)), m_treeMode(treeMode)
{
    ...
    UncheckedSetKey(key, static_cast<unsigned int>(keyLength), MakeParameters
        (SA_Name::DigestSize(),(int)digestSize)
        (SA_Name::TreeMode(),treeMode)
        (SA_Name::Salt(), ConstByteArrayParameter(salt, saltLength))
        (SA_Name::Personalization(), ConstByteArrayParameter(personalization, personalizationLength)));
}
...
void BLAKE2s::UncheckedSetKey(const byte *key, unsigned int length, const SA_CryptoPP::NameValuePairs& params)
{
    if (key && length)
    {
        m_key.New(BLOCKSIZE);
    ...
    m_digestSize = static_cast<unsigned int>(params.GetIntValueWithDefault(
                       SA_Name::DigestSize(), static_cast<int>(m_digestSize)));

    m_state.Reset();
    m_block.Reset(m_digestSize, m_keyLength);
    (void)params.GetValue(SA_Name::TreeMode(), m_treeMode);

    ConstByteArrayParameter t;
    if (params.GetValue(SA_Name::Salt(), t) && t.begin() && t.size())
        memcpy_s(m_block.salt(), SALTSIZE, t.begin(), t.size());

    if (params.GetValue(SA_Name::Personalization(), t) && t.begin() && t.size())
        memcpy_s(m_block.personalization(), PERSONALIZATIONSIZE, t.begin(), t.size());

    Restart();
}

void BLAKE2b::UncheckedSetKey(const byte *key, unsigned int length, const SA_CryptoPP::NameValuePairs& params)
{
    if (key && length)
    {
        m_key.New(BLOCKSIZE);
