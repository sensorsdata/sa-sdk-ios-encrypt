...
void HexEncoder::IsolatedInitialize(const NameValuePairs &parameters)
{
	...
	m_filter->Initialize(CombinedNameValuePairs(
		parameters,
		MakeParameters(SA_Name::EncodingLookupArray(), uppercase ? &s_vecUpper[0] : &s_vecLower[0], false)(SA_Name::Log2Base(), 4, true)));
}
...
void HexDecoder::IsolatedInitialize(const NameValuePairs &parameters)
{
	BaseN_Decoder::IsolatedInitialize(CombinedNameValuePairs(
		parameters,
		MakeParameters(SA_Name::DecodingLookupArray(), GetDefaultDecodingLookupArray(), false)(SA_Name::Log2Base(), 4, true)));
}
