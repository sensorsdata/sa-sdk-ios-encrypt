...
void BenchMark(const char *name, StreamTransformation &cipher, double timeTotal)
{
	...
	AlignedSecByteBlock buf(BUF_SIZE);
    SA_Test::GlobalRNG().GenerateBlock(buf, BUF_SIZE);
	buf.SetMark(16);
...
void BenchmarkWithCommand(int argc, const char* const argv[])
{
	...
	float runningTime(argc >= 3 ? SA_Test::StringToValue<float, true>(argv[2]) : 1.0f);
	float cpuFreq(argc >= 4 ? SA_Test::StringToValue<float, true>(argv[3])*float(1e9) : 0.0f);
	std::string algoName(argc >= 5 ? argv[4] : "");

	if (command == "b")  // All benchmarks
		Benchmark(SA_Test::All, runningTime, cpuFreq);
	else if (command == "b4")  // Public key algorithms over EC
        SA_Test::Benchmark(SA_Test::PublicKeyEC, runningTime, cpuFreq);
	else if (command == "b3")  // Public key algorithms
        SA_Test::Benchmark(SA_Test::PublicKey, runningTime, cpuFreq);
	else if (command == "b2")  // Shared key algorithms
        SA_Test::Benchmark(SA_Test::SharedKey, runningTime, cpuFreq);
	else if (command == "b1")  // Unkeyed algorithms
        SA_Test::Benchmark(SA_Test::Unkeyed, runningTime, cpuFreq);
}

void Benchmark(SA_Test::TestClass suites, double t, double hertz)
{
	g_allocatedTime = t;
	g_hertz = hertz;
