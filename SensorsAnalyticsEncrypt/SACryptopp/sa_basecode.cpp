...
{
	parameters.GetRequiredParameter("BaseN_Encoder", SA_Name::EncodingLookupArray(), m_alphabet);

	parameters.GetRequiredIntParameter("BaseN_Encoder", SA_Name::Log2Base(), m_bitsPerChar);
	if (m_bitsPerChar <= 0 || m_bitsPerChar >= 8)
		throw InvalidArgument("BaseN_Encoder: Log2Base must be between 1 and 7 inclusive");

	byte padding;
	bool pad;
	if (parameters.GetValue(SA_Name::PaddingByte(), padding))
		pad = parameters.GetValueWithDefault(SA_Name::Pad(), true);
	else
		pad = false;
...
{
	parameters.GetRequiredParameter("BaseN_Decoder", SA_Name::DecodingLookupArray(), m_lookup);

	parameters.GetRequiredIntParameter("BaseN_Decoder", SA_Name::Log2Base(), m_bitsPerChar);
	if (m_bitsPerChar <= 0 || m_bitsPerChar >= 8)
...
{
	m_groupSize = parameters.GetIntValueWithDefault(SA_Name::GroupSize(), 0);
	ConstByteArrayParameter separator, terminator;
	if (m_groupSize)
		parameters.GetRequiredParameter("Grouper", SA_Name::Separator(), separator);
	else
		parameters.GetValue(SA_Name::Separator(), separator);
	parameters.GetValue(SA_Name::Terminator(), terminator);
