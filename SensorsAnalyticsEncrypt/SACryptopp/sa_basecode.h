...
public:
		...
		, m_outputBlockSize(0), m_bytePos(0), m_bitPos(0)
	{
		...
		BaseN_Encoder::IsolatedInitialize(
			MakeParameters
				(SA_Name::EncodingLookupArray(), alphabet)
				(SA_Name::Log2Base(), log2base)
				(SA_Name::Pad(), padding != -1)
				(SA_Name::PaddingByte(), byte(padding)));
	}
...
public:
		...
		, m_outputBlockSize(0), m_bytePos(0), m_bitPos(0)
	{
		...
		BaseN_Decoder::IsolatedInitialize(
			MakeParameters
				(SA_Name::DecodingLookupArray(), lookup)
				(SA_Name::Log2Base(), log2base));
	}
...
public:
		...
		: m_groupSize(0), m_counter(0)
	{
		...
		Grouper::IsolatedInitialize(
			MakeParameters
				(SA_Name::GroupSize(), groupSize)
				(SA_Name::Separator(), ConstByteArrayParameter(separator))
				(SA_Name::Terminator(), ConstByteArrayParameter(terminator)));
	}
