...
static const byte s_moduleMac[SA_CryptoPP::HMAC<SA_CryptoPP::SHA1>::DIGESTSIZE] = CRYPTOPP_DUMMY_DLL_MAC;
CRYPTOPP_COMPILE_ASSERT(sizeof(s_moduleMac) == SA_CryptoPP::SHA1::DIGESTSIZE);
...
//	MeterFilter verifier(new FileSink("c:\\dt.tmp"));
	...
	if (!VerifyBufsEqual(expectedModuleMac, actualMac, macSize))
	{
		...
		verifier.Initialize(MakeParameters(SA_Name::OutputBuffer(), ByteArrayParameter(actualMac, (unsigned int)actualMac.size())));
//		verifier.Initialize(MakeParameters(SA_Name::OutputFileName(), (const char *)"c:\\dt2.tmp"));
		verifier.AddRangeToSkip(0, checksumPos, checksumSize);
		verifier.AddRangeToSkip(0, certificateTableDirectoryPos, certificateTableDirectorySize);
		verifier.AddRangeToSkip(0, certificateTablePos, certificateTableSize);
		verifier.AddRangeToSkip(0, macFileLocation, macSize);
...
{
	SA_CryptoPP::DoPowerUpSelfTest(NULLPTR, s_moduleMac);
}
                      ...
                      LPVOID /*lpReserved*/)
{
	...
	{
		SA_CryptoPP::s_hModule = (HMODULE)hModule;
		SA_CryptoPP::DoDllPowerUpSelfTest();
	}
    return TRUE;
}
