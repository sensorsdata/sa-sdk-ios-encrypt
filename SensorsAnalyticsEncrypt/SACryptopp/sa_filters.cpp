...
void ArraySink::IsolatedInitialize(const NameValuePairs &parameters)
{
	...
	if (!parameters.GetValue(SA_Name::OutputBuffer(), array))
	...
	: FilterWithBufferedInput(attachment), m_cipher(c), m_padding(DEFAULT_PADDING)
{
	...
	FilterWithBufferedInput::IsolatedInitialize(
		MakeParameters
			(SA_Name::BlockPaddingScheme(), padding));
}
...
{
	BlockPaddingScheme padding = parameters.GetValueWithDefault(SA_Name::BlockPaddingScheme(), DEFAULT_PADDING);
...
{
	m_putMessage = parameters.GetValueWithDefault(SA_Name::PutMessage(), false);
	int s = parameters.GetIntValueWithDefault(SA_Name::TruncatedDigestSize(), -1);
	...
	, m_hashModule(hm), m_flags(0), m_digestSize(0), m_verified(false)
{
	FilterWithBufferedInput::IsolatedInitialize(
		MakeParameters
			(SA_Name::HashVerificationFilterFlags(), flags)
			(SA_Name::TruncatedDigestSize(), truncatedDigestSize));
}
...
{
	m_flags = parameters.GetValueWithDefault(SA_Name::HashVerificationFilterFlags(), (word32)DEFAULT_FLAGS);
	int s = parameters.GetIntValueWithDefault(SA_Name::TruncatedDigestSize(), -1);
	m_digestSize = s < 0 ? m_hashModule.DigestSize() : s;
	m_verified = false;
	...
	, m_streamFilter(c, new OutputProxy(*this, false), padding, true)
{
	...
	FilterWithBufferedInput::IsolatedInitialize(
		MakeParameters
			(SA_Name::BlockPaddingScheme(), padding)
			(SA_Name::AuthenticatedDecryptionFilterFlags(), flags)
			(SA_Name::TruncatedDigestSize(), truncatedDigestSize));
}
...
{
	word32 flags = parameters.GetValueWithDefault(SA_Name::AuthenticatedDecryptionFilterFlags(), (word32)DEFAULT_FLAGS);

	m_hashVerifier.Initialize(CombinedNameValuePairs(parameters, MakeParameters(SA_Name::HashVerificationFilterFlags(), flags)));
	...
	, m_verifier(verifier), m_flags(0), m_verified(0)
{
	FilterWithBufferedInput::IsolatedInitialize(
		MakeParameters
			(SA_Name::SignatureVerificationFilterFlags(), flags));
}
...
{
	m_flags = parameters.GetValueWithDefault(SA_Name::SignatureVerificationFilterFlags(), (word32)DEFAULT_FLAGS);
...
void StringStore::StoreInitialize(const NameValuePairs &parameters)
{
	...
	if (!parameters.GetValue(SA_Name::InputBuffer(), array))
