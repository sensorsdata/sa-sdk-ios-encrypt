...
bool ValidateRSA_Sign()
{
...

	{
		...
		SA_Weak::RSASSA_PKCS1v15_MD2_Signer rsaPriv(keys);
		SA_Weak::RSASSA_PKCS1v15_MD2_Verifier rsaPub(rsaPriv);
...
{
	ECDSA<ECP, SHA1>::Signer spriv(GlobalRNG(), SA_ASN1::secp192r1());
...
bool ValidateEC2N_Sign()
{
	...
	ECDSA<EC2N, SHA1>::Signer spriv(GlobalRNG(), SA_ASN1::sect193r1());
...
bool ValidateECGDSA(bool thorough)
{
	...
	if (thorough)
	{
		const OID oid = SA_ASN1::brainpoolP192r1();
	...
	if (thorough)
	{
		const OID oid = SA_ASN1::brainpoolP256r1();
		DL_GroupParameters_EC<ECP> params(oid);
	...
	if (thorough)
	{
		const OID oid = SA_ASN1::brainpoolP320r1();
		DL_GroupParameters_EC<ECP> params(oid);
	...
	{
		const OID oid = SA_ASN1::brainpoolP512r1();
		DL_GroupParameters_EC<ECP> params(oid);
