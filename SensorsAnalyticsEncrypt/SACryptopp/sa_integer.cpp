...
#include "sa_oids.h"
#include "sa_words.h"
#include "sa_pubkey.h"        // for P1363_KDF2
#include "sa_sha.h"
#include "sa_cpu.h"
#include "sa_misc.h"
...
bool Integer::GenerateRandomNoThrow(RandomNumberGenerator &i_rng, const NameValuePairs &params)
{
	...
	if (params.GetValue(SA_Name::Seed(), seed))
	{
		ByteQueue bq;
	...
	switch (rnType)
	{
		...
		case PRIME:
		{
			const PrimeSelector *pSelector = params.GetValueWithDefault(SA_Name::PointerToPrimeSelector(), (const PrimeSelector *)NULLPTR);
...
int Integer::PositiveCompare(const Integer& t) const
{
	...
	if (size != tSize)
		...
		return SA_CryptoPP::Compare(reg, t.reg, size);
}
...
void ModularArithmetic::DEREncode(BufferedTransformation &bt) const
{
	...
	SA_ASN1::prime_field().DEREncode(seq);
...
const Integer& ModularArithmetic::Half(const Integer &a) const
{
	...
	{
		SA_CryptoPP::DivideByPower2Mod(m_result.reg.begin(), a.reg, 1, m_modulus.reg, a.reg.size());
		return m_result;
	}
	else
...
const Integer& ModularArithmetic::Add(const Integer &a, const Integer &b) const
{
	...
	{
		if (SA_CryptoPP::Add(m_result.reg.begin(), a.reg, b.reg, a.reg.size())
			|| Compare(m_result.reg, m_modulus.reg, a.reg.size()) >= 0)
		{
			SA_CryptoPP::Subtract(m_result.reg.begin(), m_result.reg, m_modulus.reg, a.reg.size());
		}
		return m_result;
	}
	else
...
Integer& ModularArithmetic::Accumulate(Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (SA_CryptoPP::Add(a.reg, a.reg, b.reg, a.reg.size())
			|| Compare(a.reg, m_modulus.reg, a.reg.size()) >= 0)
		{
			SA_CryptoPP::Subtract(a.reg, a.reg, m_modulus.reg, a.reg.size());
		}
	}
	else
	{
...
const Integer& ModularArithmetic::Subtract(const Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (SA_CryptoPP::Subtract(m_result.reg.begin(), a.reg, b.reg, a.reg.size()))
			SA_CryptoPP::Add(m_result.reg.begin(), m_result.reg, m_modulus.reg, a.reg.size());
		return m_result;
	}
	else
	{
...
Integer& ModularArithmetic::Reduce(Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (SA_CryptoPP::Subtract(a.reg, a.reg, b.reg, a.reg.size()))
			SA_CryptoPP::Add(a.reg, a.reg, m_modulus.reg, a.reg.size());
	}
	else
	{
		a-=b;
...
const Integer& ModularArithmetic::Inverse(const Integer &a) const
{
	...
	if (SA_CryptoPP::Subtract(m_result.reg.begin(), m_result.reg, a.reg, a.reg.size()))
...
const Integer& MontgomeryRepresentation::Square(const Integer &a) const
{
	...
	SA_CryptoPP::Square(T, T+2*N, a.reg, a.reg.size());
