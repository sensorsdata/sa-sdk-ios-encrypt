...
extern "C" {
word32 sa_s_sosemanukMulTables[512] = {
...
#define SSE2_STEP(i, j)	\
	...
	AS2(	mov		[s(i+0)], AS_REG_6d)\
...
#ifdef _MSC_VER
		AS2(	lea		WORD_REG(si), sa_s_sosemanukMulTables)
#else
		...
		ATT_PREFIX
			:
			: "a" (m_state.data()), "c" (iterationCount), "S" (sa_s_sosemanukMulTables), "D" (output), "d" (input)
	#if CRYPTOPP_BOOL_X64
			, "r" (workspace.data())
	...
	{
#if (CRYPTOPP_BOOL_X86 || CRYPTOPP_BOOL_X32 || CRYPTOPP_BOOL_X64) && !defined(CRYPTOPP_DISABLE_SOSEMANUK_ASM)
#define MUL_A(x)    (x = (rotlConstant<8>(x)), x ^ sa_s_sosemanukMulTables[byte(x)])
#else
#define MUL_A(x)    (((x) << 8) ^ sa_s_sosemanukMulTables[(x) >> 24])
#endif

#define DIV_A(x)    (((x) >> 8) ^ sa_s_sosemanukMulTables[256 + byte(x)])
