...
void RandomizedTransfer(BufferedTransformation &source, BufferedTransformation &target, bool finish, const std::string &channel=DEFAULT_CHANNEL)
{
	...
	{
		byte buf[4096+64];
		size_t start = SA_Test::GlobalRNG().GenerateWord32(0, 63);
		size_t len = SA_Test::GlobalRNG().GenerateWord32(1, UnsignedMin(4096U, 3*source.MaxRetrievable()/2));
...
public:
	...
	virtual bool GetVoidValue(const char *name, const std::type_info &valueType, void *pValue) const
	{
		...
		if (i == m_data.end())
		{
			if (std::string(name) == SA_Name::DigestSize() && valueType == typeid(int))
			{
				i = m_data.find("MAC");
				if (i == m_data.end())
...
{
	if (!pub.Validate(SA_Test::GlobalRNG(), 2U+!!s_thorough))
		SignalTestFailure();
	if (!priv.Validate(SA_Test::GlobalRNG(), 2U+!!s_thorough))
		SignalTestFailure();
...
void TestSignatureScheme(TestData &v)
{
	...
	if (test == "GenerateKey")
	{
		signer->AccessPrivateKey().GenerateRandom(SA_Test::GlobalRNG(), pairs);
	...
	else
	{
		...
		{
			if (!verifier->GetMaterial().Validate(SA_Test::GlobalRNG(), 3))
				SignalTestFailure();
			return;
		}
	...
	if (test == "GenerateKey" || test == "KeyPairValidAndConsistent")
	{
		...
		StringSource ss(msg, sizeof(msg), true, new SignerFilter(SA_Test::GlobalRNG(), *signer, new Redirector(verifierFilter)));
	}
	else if (test == "Sign")
	{
		SignerFilter f(SA_Test::GlobalRNG(), *signer, new HexEncoder(new FileSink(std::cout)));
	...
	else if (test == "DeterministicSign")
	{
		...
		SignerFilter f(SA_Test::GlobalRNG(), *signer, new StringSink(signature));
		StringSource ss(GetDecodedDatum(v, "Message"), true, new Redirector(f));
...
void TestAsymmetricCipher(TestData &v)
{
	...
	if (test == "DecryptMatch")
	{
		...
		StringSource ss(GetDecodedDatum(v, "Ciphertext"), true, new PK_DecryptorFilter(SA_Test::GlobalRNG(), *decryptor, new StringSink(decrypted)));
...
void TestSymmetricCipher(TestData &v, const NameValuePairs &overrideParameters)
{
	...
	if (test == "Encrypt" || test == "EncryptXorDigest" || test == "Resync" || test == "EncryptionMCT" || test == "DecryptionMCT")
	{
		...
		int paddingScheme = pairs.GetIntValueWithDefault(SA_Name::BlockPaddingScheme(), 0);

		ConstByteArrayParameter iv;
		if (pairs.GetValue(SA_Name::IV(), iv) && iv.size() != encryptor->IVSize())
			SignalTestFailure();
...
void TestAuthenticatedSymmetricCipher(TestData &v, const NameValuePairs &overrideParameters)
{
	...
	if (test == "Encrypt" || test == "EncryptXorDigest" || test == "NotVerify")
	{
		...
		bool macAtBegin = !mac.empty() && !SA_Test::GlobalRNG().GenerateBit();	// test both ways randomly
...
void TestDigestOrMAC(TestData &v, bool testDigest)
{
	...
	if (test == "Verify" || test == "VerifyTruncated" || test == "NotVerify")
	{
		...
		if (test == "VerifyTruncated")
			digestSize = pairs.GetIntValueWithDefault(SA_Name::DigestSize(), digestSize);
...
void TestDataFile(std::string filename, const NameValuePairs &overrideParameters, unsigned int &totalTests, unsigned int &failedTests)
{
	...
	while (file)
	{
		...
		if (name == "Test" && (s_thorough || v["SlowTest"] != "1"))
		{
			...
			catch (const SA_CryptoPP::Exception &e)
			{
				std::cout << "\nSA_CryptoPP::Exception caught: " << e.what() << std::endl;
			}
