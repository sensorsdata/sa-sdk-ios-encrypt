...
void BenchMarkEncryption(const char *name, PK_Encryptor &key, double timeTotal, bool pc = false)
{
    ...
    SA_Test::GlobalRNG().GenerateBlock(plaintext, len);
	...
	do
	{
		key.Encrypt(SA_Test::GlobalRNG(), plaintext, len, ciphertext);
...
void BenchMarkDecryption(const char *name, PK_Decryptor &priv, PK_Encryptor &pub, double timeTotal)
{
    ...
    SA_Test::GlobalRNG().GenerateBlock(plaintext, len);
	pub.Encrypt(SA_Test::GlobalRNG(), plaintext, len, ciphertext);
	...
	do
	{
		priv.Decrypt(SA_Test::GlobalRNG(), ciphertext, ciphertext.size(), plaintext);
		++i; timeTaken = timer.ElapsedTimeAsDouble();
	}
	while (timeTaken < timeTotal);
...
void BenchMarkSigning(const char *name, PK_Signer &key, double timeTotal, bool pc=false)
{
    ...
    SA_Test::GlobalRNG().GenerateBlock(message, len);
	...
	do
	{
		(void)key.SignMessage(SA_Test::GlobalRNG(), message, len, signature);
		++i; timeTaken = timer.ElapsedTimeAsDouble();
	}
	while (timeTaken < timeTotal);
...
void BenchMarkVerification(const char *name, const PK_Signer &priv, PK_Verifier &pub, double timeTotal, bool pc=false)
{
    ...
    SA_Test::GlobalRNG().GenerateBlock(message, len);
	priv.SignMessage(SA_Test::GlobalRNG(), message, len, signature);
...
void BenchMarkKeyGen(const char *name, SimpleKeyAgreementDomain &d, double timeTotal, bool pc=false)
{
	...
	do
	{
		d.GenerateKeyPair(SA_Test::GlobalRNG(), priv, pub);
		++i; timeTaken = timer.ElapsedTimeAsDouble();
	}
	while (timeTaken < timeTotal);
...
void BenchMarkKeyGen(const char *name, AuthenticatedKeyAgreementDomain &d, double timeTotal, bool pc=false)
{
	...
	do
	{
		d.GenerateEphemeralKeyPair(SA_Test::GlobalRNG(), priv, pub);
		++i; timeTaken = timer.ElapsedTimeAsDouble();
	}
	while (timeTaken < timeTotal);
...
void BenchMarkAgreement(const char *name, SimpleKeyAgreementDomain &d, double timeTotal, bool pc=false)
{
	...
	d.GenerateKeyPair(SA_Test::GlobalRNG(), priv1, pub1);
	d.GenerateKeyPair(SA_Test::GlobalRNG(), priv2, pub2);
...
void BenchMarkAgreement(const char *name, AuthenticatedKeyAgreementDomain &d, double timeTotal, bool pc=false)
{
	...
	d.GenerateStaticKeyPair(SA_Test::GlobalRNG(), spriv1, spub1);
	d.GenerateStaticKeyPair(SA_Test::GlobalRNG(), spriv2, spub2);
	d.GenerateEphemeralKeyPair(SA_Test::GlobalRNG(), epriv1, epub1);
	d.GenerateEphemeralKeyPair(SA_Test::GlobalRNG(), epriv2, epub2);
	SecByteBlock val(d.AgreedValueLength());
...
void Benchmark4(double t, double hertz)
{
	...
	std::cout << "\n<TBODY style=\"background: white;\">";
	{
		ed25519::Signer sign(SA_Test::GlobalRNG());
		ed25519::Verifier verify(sign);
		x25519 agree(SA_Test::GlobalRNG());
	...
	std::cout << "\n<TBODY style=\"background: yellow;\">";
	{
		ECIES<ECP>::Decryptor cpriv(SA_Test::GlobalRNG(), SA_ASN1::secp256k1());
		...
		ECGDSA<ECP, SHA1>::Signer spriv3(SA_Test::GlobalRNG(), SA_ASN1::secp256k1());
		ECGDSA<ECP, SHA1>::Verifier spub3(spriv3);
		ECDH<ECP>::Domain ecdhc(SA_ASN1::secp256k1());
		ECMQV<ECP>::Domain ecmqvc(SA_ASN1::secp256k1());
	...
	std::cout << "\n<TBODY style=\"background: white;\">";
	{
		ECIES<EC2N>::Decryptor cpriv(SA_Test::GlobalRNG(), SA_ASN1::sect233r1());
		...
		ECGDSA<EC2N, SHA1>::Signer spriv3(SA_Test::GlobalRNG(), SA_ASN1::sect233r1());
		ECGDSA<EC2N, SHA1>::Verifier spub3(spriv3);
		ECDH<EC2N>::Domain ecdhc(SA_ASN1::sect233r1());
		ECMQV<EC2N>::Domain ecmqvc(SA_ASN1::sect233r1());
