...
bool ValidateARC4()
{
	...
	member_ptr<SA_Weak::ARC4> arc4;
	bool pass=true, fail;
	unsigned int i;

	std::cout << "\nARC4 validation suite running...\n\n";

	arc4.reset(new SA_Weak::ARC4(Key0, sizeof(Key0)));
	...
	arc4.reset(new SA_Weak::ARC4(Key1, sizeof(Key1)));
	...
	arc4.reset(new SA_Weak::ARC4(Key2, sizeof(Key2)));
	...
	arc4.reset(new SA_Weak::ARC4(Key3, sizeof(Key3)));
	...
	arc4.reset(new SA_Weak::ARC4(Key4, sizeof(Key4)));
...
{
	IsolatedInitialize(MakeParameters(SA_Name::InsertLineBreaks(), true)(SA_Name::MaxLineLength(), 72));
}

void MyEncoder::IsolatedInitialize(const NameValuePairs &parameters)
{
	bool insertLineBreaks = parameters.GetValueWithDefault(SA_Name::InsertLineBreaks(), true);
	int maxLineLength = parameters.GetIntValueWithDefault(SA_Name::MaxLineLength(), 72);
	...
	m_filter->Initialize(CombinedNameValuePairs(
		parameters,
		MakeParameters(SA_Name::EncodingLookupArray(), (const byte *)&stars[0], false)
			(SA_Name::PaddingByte(), padding)
			(SA_Name::GroupSize(), insertLineBreaks ? maxLineLength : 0)
			(SA_Name::Separator(), ConstByteArrayParameter(lineBreak))
			(SA_Name::Terminator(), ConstByteArrayParameter(lineBreak))
			(SA_Name::Log2Base(), 6, true)));
}
...
void MyDecoder::IsolatedInitialize(const NameValuePairs &parameters)
{
	BaseN_Decoder::IsolatedInitialize(CombinedNameValuePairs(
		parameters,
		MakeParameters(SA_Name::DecodingLookupArray(), GetDecodingLookupArray(), false)(SA_Name::Log2Base(), 6, true)));
}
...
bool ValidateEncoder()
{
	...
	AlgorithmParameters eparams = MakeParameters(SA_Name::EncodingLookupArray(),(const byte*)alphabet)
	                                            (SA_Name::InsertLineBreaks(), false);
	...
	AlgorithmParameters dparams = MakeParameters(SA_Name::DecodingLookupArray(),(const int*)lookup);
	...
	// at https://github.com/weidai11/cryptopp/issues/562.
	{
		...
		AlgorithmParameters dparams2 = MakeParameters(SA_Name::DecodingLookupArray(),(const int*)lookup);
...
bool ValidateGCM()
{
	...
	std::cout << "\n2K tables:";
	bool pass = RunTestDataFile("TestVectors/gcm.txt", MakeParameters(SA_Name::TableSize(), (int)2048));
	std::cout << "\n64K tables:";
	return RunTestDataFile("TestVectors/gcm.txt", MakeParameters(SA_Name::TableSize(), (int)64*1024)) && pass;
}
