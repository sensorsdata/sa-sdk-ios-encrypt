...
void InvertibleRSAFunction::GenerateRandom(RandomNumberGenerator &rng, const NameValuePairs &alg)
{
	...
	alg.GetIntValue(SA_Name::ModulusSize(), modulusSize) || alg.GetIntValue(SA_Name::KeySize(), modulusSize);

	CRYPTOPP_ASSERT(modulusSize >= 16);
	if (modulusSize < 16)
		throw InvalidArgument("InvertibleRSAFunction: specified modulus size is too small");

	m_e = alg.GetValueWithDefault(SA_Name::PublicExponent(), Integer(17));
	...
	AlgorithmParameters primeParam = MakeParametersForTwoPrimesOfEqualSize(modulusSize)
		(SA_Name::PointerToPrimeSelector(), selector.GetSelectorPointer());
...
{
	GenerateRandom(rng, MakeParameters(SA_Name::ModulusSize(), (int)keybits)(SA_Name::PublicExponent(), e+e.IsEven()));
}
