...
void BenchMarkByName2(const char *factoryName, size_t keyLength=0, const char *displayName=NULLPTR, const NameValuePairs &params = g_nullNameValuePairs)
{
	...
	obj->SetKey(defaultKey, keyLength, CombinedNameValuePairs(params, MakeParameters(SA_Name::IV(), ConstByteArrayParameter(defaultKey, obj->IVSize()), false)));
	BenchMark(name.c_str(), *static_cast<T_Interface *>(obj.get()), g_allocatedTime);
	BenchMarkKeying(*obj, keyLength, CombinedNameValuePairs(params, MakeParameters(SA_Name::IV(), ConstByteArrayParameter(defaultKey, obj->IVSize()), false)));
}
...
void Benchmark2(double t, double hertz)
{
...
#endif
		{
			BenchMarkByName2<AuthenticatedSymmetricCipher, MessageAuthenticationCode>("AES/GCM", 0, "GMAC(AES) (2K tables)", MakeParameters(SA_Name::TableSize(), 2048));
			BenchMarkByName2<AuthenticatedSymmetricCipher, MessageAuthenticationCode>("AES/GCM", 0, "GMAC(AES) (64K tables)", MakeParameters(SA_Name::TableSize(), 64 * 1024));
		}
	...
	std::cout << "\n<TBODY style=\"background: yellow;\">";
	{
		...
		BenchMarkByName<SymmetricCipher>("Salsa20", 0, "Salsa20/12", MakeParameters(SA_Name::Rounds(), 12));
		BenchMarkByName<SymmetricCipher>("Salsa20", 0, "Salsa20/8", MakeParameters(SA_Name::Rounds(), 8));
		BenchMarkByName<SymmetricCipher>("ChaCha", 0, "ChaCha20");
		BenchMarkByName<SymmetricCipher>("ChaCha", 0, "ChaCha12", MakeParameters(SA_Name::Rounds(), 12));
		BenchMarkByName<SymmetricCipher>("ChaCha", 0, "ChaCha8", MakeParameters(SA_Name::Rounds(), 8));
...
#endif
		{
			BenchMarkByName2<AuthenticatedSymmetricCipher, AuthenticatedSymmetricCipher>("AES/GCM", 0, "AES/GCM (2K tables)", MakeParameters(SA_Name::TableSize(), 2048));
			BenchMarkByName2<AuthenticatedSymmetricCipher, AuthenticatedSymmetricCipher>("AES/GCM", 0, "AES/GCM (64K tables)", MakeParameters(SA_Name::TableSize(), 64 * 1024));
		}
